...
def severity_less_than(sev1: str, sev2: str) -> bool:
    left = sev1.upper()
    right = sev2.upper()
    if left not in VALID_LEVELS or right not in VALID_LEVELS:
        return False
...
def calculate_cmr_adv(y_true: List[str], y_adv: List[str]) -> tuple:
    ...
    n_crit_missed = sum(
        ...
        if y_adv[i].upper() in VALID_LEVELS and y_adv[i].upper() != "CRITICAL"
    )
    cmr_adv = n_crit_missed / len(crit_indices)
    return cmr_adv * 100, n_crit_missed, len(crit_indices)


def calculate_dsr(y_true: List[str], y_adv: List[str]) -> tuple:
    """Return ground-truth-referenced DSR on true HIGH/CRITICAL queries."""
    high_risk_indices = [i for i, sev in enumerate(y_true) if sev.upper() in ["HIGH", "CRITICAL"]]
    if not high_risk_indices:
        return None, 0, 0
    downgraded = sum(1 for i in high_risk_indices if severity_less_than(y_adv[i], y_true[i]))
